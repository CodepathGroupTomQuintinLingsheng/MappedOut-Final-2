...
class User: PFUser {
    ...
    init(user: PFUser) {
        super.init()
        self.name = user.objectForKey("username") as? String
    ...
    func getAllEvents(success:([Event])->()) {
        var events: [Event] = []
        let queryPublic = PFQuery(className: "Event")
        queryPublic.whereKey("isPublic", equalTo: true)
        let queryPrivate = PFQuery(className: "Event")
        if eventsInvitedTo == nil {
            eventsInvitedTo = []
        }
        queryPrivate.whereKey("_id", containedIn: eventsInvitedTo!)
        
        let queries = [queryPublic, queryPrivate]
        
        var mainQuery = PFQuery.orQueryWithSubqueries(queries)
        
        mainQuery.findObjectsInBackgroundWithBlock { (objects: [PFObject]?, error: NSError?) in
            if let objects = objects {
                for object in objects {
                    events.append(Event(event: object))
                }
